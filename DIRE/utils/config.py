...
class DefaultConfigs(ABC):
    ...
    seed = 3407
    arch = "resnet50"
    datasets = ["GenImage-SDV14"]
    datasets_test = ["GenImage-SDV14"]
    # datasets_test = ["GenImage-glide-hard"]
    mode = "binary"
    class_bal = False
    batch_size = 64
    loadSize = 256
    ...
    # paths information original
    # root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    # dataset_root = os.path.join(root_dir, "data")
    # exp_root = os.path.join(root_dir, "data", "exp")

    # base training
    root_dir = os.path.abspath("/data/datasets/DiffusionForensics")
    dataset_root = os.path.join(root_dir, "dire")
    exp_root = os.path.join(root_dir, "dire", "exp")
...
parser.add_argument("--gpus", default=[2], type=int, nargs="+")
